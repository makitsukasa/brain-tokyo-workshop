...
class Ind():
	...
	def tree(self):
		...
		for i in range(len(self.node[0])):
			G.node(str(int(self.node[0][i])), label=["", "i", "o", "h", "b"][int(self.node[1][i])])
...
def act(weights, aVec, nInput, nOutput, inPattern):
	...
	nodeAct_orig = np.zeros((nSamples,nNodes))
	nodeAct_orig[:,0] = 1 # Bias activation
	nodeAct_orig[:,1:nInput+1] = inPattern

	rawAct_orig = np.zeros((nSamples,nNodes))

	# Propagate signal through hidden to output nodes
	iNode = nInput+1
	for iNode in range(nInput+1,nNodes):
		rawAct_orig[:,iNode] = np.dot(nodeAct_orig, wMat[:,iNode]).squeeze()
		# rawAct_orig = np.zeros(nSamples)
		nodeAct_orig[:,iNode] = applyAct(aVec[iNode], rawAct_orig[:,iNode])
		# print(nodeAct_orig)
	output = nodeAct_orig[:,-nOutput:]
	...
	if not np.allclose(output, nodeAct[:,-nOutput:]):
		...
		for i in range(len(sa[0])):
			print(sa[0][i], end=", ")
			print(sa[1][i], end=", ")
			print((nodeAct[:,-nOutput:])[sa[0][i], sa[1][i]], end=", ")
			print(output[sa[0][i], sa[1][i]], end=", ")
			print("")
		exit()
